...
class BaccoCalculator(object):
    ...
    def __init__(self, log10k_min=np.log10(0.008), log10k_max=np.log10(0.5), nk_per_decade=20,
                 ...
                 allow_halofit_extrapolation_for_shear_on_k=False,dx = 0.01,Fr_order = 1):
    ...
    def update_pk(self, cosmo, bcmpar=None, **kwargs):
        ...
        if len(without_bounds_mpk) == 0: # if all parameters are within baccoemu narrower bounds, run Carlos' code.
            ...
            drk_perturbation_term_dictionary = {}  # storing derivatives*delta_theta for all parameters
            ...
            if (self.taylor_order >= 2):
                ...
                dr2k_dvariable_dictionary = {} # storing all derivatives
                dr2k_perturbation_term_dictionary = {}  # storing derivatives*delta_theta for all parameters
                ...
                for elements in  itertools.product(without_bounds_mpk,without_bounds_mpk):
                		...
                		dr2k_dvariable_dictionary['dr2k_d' + par_name_1+par_name_2]  = 0.5 *  self.second_order_central_derivator_diagonal(self._get_dr2k_dparameter,to_start_value_mpk[par_name_1],to_start_value_mpk[par_name_2],dx=self.dx,args=(k_at_start,cosmopars_start_mpk.copy(),par_name_1,par_name_2))#[1]
                             # array of length k with the power spectrum derivative with respect to the desired parameter
                
                        if par_name_1 != par_name_2: # derivata mista
                        	dr2k_dvariable_dictionary['dr2k_d' + par_name_1+par_name_2]  =   self.second_order_mixed_derivator_mixed(self._get_dr2k_dparameter,to_start_value_mpk[par_name_1],to_start_value_mpk[par_name_2],dx=self.dx,dy=self.dx,args=(k_at_start,cosmopars_start_mpk.copy(),par_name_1,par_name_2))#[1]
                             # 
                
                	dr2k_perturbation_term_dictionary[par_name_1+'_'+par_name_2] = dr2k_dvariable_dictionary['dr2k_d' + par_name_1+par_name_2] * ( cosmopars_predict_mpk.copy()[par_name_1] - cosmopars_start_mpk.copy()[par_name_1] ) * ( cosmopars_predict_mpk.copy()[par_name_2] - cosmopars_start_mpk.copy()[par_name_2] ) # calculate dp/dpar * delta_par 
                	
                	total_perturbation_rk = total_perturbation_rk + dr2k_perturbation_term_dictionary[par_name_1+'_'+par_name_2] # add all the perturbations together, at first order it is linear	    
               
               
            
            self.drk_dvariable_dictionary = drk_dvariable_dictionary
            self.drk_perturbation_term_dictionary = drk_perturbation_term_dictionary
            
            self.dr2k_dvariable_dictionary = dr2k_dvariable_dictionary
            self.dr2k_perturbation_term_dictionary = dr2k_perturbation_term_dictionary
    ...
    def _get_d2rk_dparameter(self,param_1_evaluation_point,param_2_evaluation_point,k,params_dict,param_sensitivity_name_1,param_sensitivity_name_2,):
        ...
        params_dict[param_sensitivity_name_1] = param_evaluation_point_1  #S_8 * ((S_8_sensitivity_params['omega_cold'])/0.3)**(-alpha)
        params_dict[param_sensitivity_name_2] = param_evaluation_point_2
        k, dpk_dparameter_biased  =  self.lbias.get_nonlinear_pnn(k=k, **params_dict)  #[1]/h**3#emulator.get_nonlinear_pnn(k=k, **params_dict) 
        return np.array(dpk_dparameter_biased / dpk_dparameter_biased[0])  #MIND THE RATIO and check the index! 
    ...
    def first_order_central_derivator(self, f,x_0,args=(), dx=0.01,) :
    
        '''
        Scipy's derivator is deprecated, this is my personal version, central difference with o(dx^2) precision
        '''
    
        right_edge = f(x_0+dx,*args)
        left_edge = f(x_0-dx,*args)
        num = right_edge - left_edge
        den = 2*dx
        return num/den
    
    
    def second_order_central_derivator_diagonal(self, f,x_0,args=(), dx=0.01,) :
    
        '''
        This is a UNIVARIATE second derivative, it works fine for one-variable second derivatives or 
        for the diagonal entries of the Hessian in partial differentiation cases.
        
        Scipy's derivator is deprecated, this is my personal version, central difference with o(dx^4) precision
        ...
        term_1 = f(x_0-dx,*args)
        term_2 = -2 * f(x_0,*args) 
        term_3 = f(x_0+dx,*args)
    ...
    def second_order_central_derivator_mixed(self, f,x_0,y_0,args=(), dx=0.01,dy = 0.01) :
    
        '''
        This is a BIVARIATE second derivative, used for calculating off-diagonal bivariate mixed derivatives in the Hessian, 
        useful for partial derivation cases.  
         
        Scipy's derivator is deprecated, this is my personal version, central difference, it should have o(dx^2) (?) precision
        https://www.uio.no/studier/emner/matnat/math/MAT-INF1100/h07/undervisningsmateriale/kap7.pdf
        
        x_0 and y_0 represent the couple of variables with respect to which I am taking the two derivatves. 
        ...
        term_1 = f(x_0+dx,y_0+dy,*args)
        term_2 = f(x_0+dx,y_0-dy,*args)
        term_3 = f(x_0-dx,y_0+dy,*args)
        term_4 = f(x_0-dx,y_0-dy,*args)
        
        
        num = term_1 - term_2 - term_3 + term_4  
        den = 4 * dx * dy
