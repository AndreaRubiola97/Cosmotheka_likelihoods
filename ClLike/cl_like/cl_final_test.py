...
class ClFinal(Theory):
    ...
    # def initiaize_with_provider(self, provider):
    def initialize_with_provider(self,provider):
        ...
        #chi2_val = self.provider.get_chi2()#get_is_PT_bias()
        #jeffrey_bias_dchi2_val = self.provider.get_jeffrey_bias_dchi2()
    ...
    def get_requirements(self):
        return {"ia_model": None, "bias_model": None, "is_PT_bias": None,}#"chi2_val": None,"jeffrey_bias_dchi2_val": None,}
    ...
    def must_provide(self, **requirements):
        ...
        #options = requirements.get('chi2') or {} 
        #options = requirements.get('jeffrey_bias_dchi2') or {}
        self.cl_meta = options.get("cl_meta")
        self.tracer_qs = options.get("tracer_qs")
        self.bin_properties = options.get("bin_properties")
        self.cl_data = options.get('cl_data')
        self.inv_cov = options.get('inv_cov')
        #print('self.get_chi2',self.get_chi2)
        #self.get_jeffrey_bias_dchi2 = requirements.get('jeffrey_bias_dchi2')
        ...
        return {"Limber": {"cl_meta": self.cl_meta,
                           ...
                           #'chi2': self.get_chi2,"jeffrey_bias_dchi2": self.get_jeffrey_bias_dchi2}}
                           #"chi2": self.get_chi2,"jeffrey_bias_dchi2": self.get_jeffrey_bias_dchi2
                           #}
    ...
    def calculate(self, state, want_derived=True, **pars):
        ...
        t = state["cl_theory"]#self.get_cl_theory()
        ...
        if jeffrey_bias:
           ...
           ic_g = np.dot(inv_cov, g) # (ndata, ndata) , (ndata, nbias)
           F = np.sum(g[:, None, :]*ic_g[:, :, None], axis=0) # (ndata, _, nbias) , (ndata, nbias, _) -> (nbias, nbias)
        
           dchi2_jeffrey = -np.log(np.linalg.det(F))
    ...
    def get_chi2(self,data_vec, inv_cov,jeffrey_bias):
    
        return self._current_state['chi2']
        #,self.current_state['logp'], dchi2_jeffrey
    
    def get_jeffrey_bias_dchi2(self,**pars):
        
        return self.derived._current_state['dchi2_jeffrey']#-np.log(np.linalg.det(F))
