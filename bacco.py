...
# Notice that for 2nd order extrapolation dx = dy, check and consider different steps for the two variables independently
...
class BaccoCalculator(object):
    ...
    def __init__(self, log10k_min=np.log10(0.008), log10k_max=np.log10(0.5), nk_per_decade=20,
                 ...
                 allow_halofit_extrapolation_for_shear_on_k=False,dx = 0.01,taylor_order = 1):
    ...
    def update_pk(self, cosmo, bcmpar=None, **kwargs):
        ...
        if len(without_bounds_mpk) == 0: # if all parameters are within baccoemu narrower bounds, run Carlos' code.
            ...
            if (self.taylor_order >=2):
                self.d2rk_dvariable_dictionary = d2rk_dvariable_dictionary
                self.d2rk_perturbation_term_dictionary = d2rk_perturbation_term_dictionary
    ...
    def _get_d2rk_dparameter(self,param_1_evaluation_point,param_2_evaluation_point,k,params_dict,param_sensitivity_name_1,param_sensitivity_name_2,):
        ...
        params_dict[param_sensitivity_name_1] = param_1_evaluation_point #S_8 * ((S_8_sensitivity_params['omega_cold'])/0.3)**(-alpha)
        params_dict[param_sensitivity_name_2] = param_2_evaluation_point
        k, dpk_dparameter_biased  =  self.lbias.get_nonlinear_pnn(k=k, **params_dict)  #[1]/h**3#emulator.get_nonlinear_pnn(k=k, **params_dict) 
        return np.array(dpk_dparameter_biased / dpk_dparameter_biased[0])  #MIND THE RATIO and check the index! 
    ...
    def second_order_central_derivator_diagonal(self,f,x_0,y_0,args=(), dx=0.01,) :
        ...
        term_1 = f(x_0-dx,x_0-dx,*args)
        term_2 = -2 * f(x_0,x_0,*args) 
        term_3 = f(x_0+dx,x_0+dx,*args)
